 ...
 * Toolkit, Button, BaseBuilder, Native Flow Support
 *
 * Runtime:
 * - Baileys: @whiskeysockets/baileys (latest)
 *
 * Created by RizzyFuzz @rizzbrew
 *
 * Copyright (c) 2026 RizzyFuzz
 *
...
function extractIE(text, { extract = true, hyperlink = true, citation = true, latex = true } = {}) {
	...
	const createIE = (type, ie) => {
		...
		if (type == 'latex') {
			return (async () => {
				let w = Number(ie.width);
				let h = Number(ie.height);

				if (!w || !h) {
					try {
						const response = await fetch(ie.url);
						if (response.ok) {
	...
	for (let i = 0; i < text.length; i++) {
		if (text[i] == '[' && text[i - 1] != '\\') {
			...
			if (type == 'latex') {
				...
				key = `WA_HYPERLINK_${hyperlink_index++}`;
				tag = `{{${key}}}${url}{{/${key}}}`;
				...
				key = `WA_CITATION_${citation_index - 1}`;
				tag = `{{${key}}}${url}{{/${key}}}`;
	...
	static getMp4Preview(videoBuffer, { time, result = 'buffer', resize = true, width = 300, height = 300, silent = true } = {}) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				ffmpegProcess.on('error', (err) => fail(new Error(`ffmpeg error: ${err.message}`)));
				
				inputStream.on('error', () => {});
				ffmpegProcess.stdin.on('error', () => {});

				inputStream.pipe(ffmpegProcess.stdin);
				ffmpegProcess.stdout.pipe(outputStream, { end: true });
			} catch (err) {
				return fail(err);
			}
		});
...
class BaseBuilder {
	...
	async setLocation(jpegThumbnail, name, address) {
		const buffer = Buffer.isBuffer(jpegThumbnail) ? jpegThumbnail : await Toolkit.fetchBuffer(jpegThumbnail, {}, { silent: true });
...
class ButtonV2 extends BaseBuilder {
	...
	async build(jid, { ...options } = {}) {
		let _thumbnail = null;
		if (this._image) {
			const buffer = Buffer.isBuffer(this._image) ? this._image : await Toolkit.fetchBuffer(this._image, {}, { silent: true });
			if (buffer && buffer.length) {
				_thumbnail = await sharp(buffer)
					.resize(300, 300, {
						fit: 'contain',
						background: { r: 255, g: 255, b: 255, alpha: 1 },
					})
...
class AIRich extends BaseBuilder {
	...
	addNewLayout(type, data, extra = {}) {
		this._sections.push(AIRich.newLayout(type, data, extra));
		return this;
	}
	...
	addText(text, { hyperlink = true, citation = true, latex = true } = {}) {
		...
		// WhatsApp GenAIMarkdownTextUXPrimitive has a limit around ~1024 characters per section.
		const MAX_LEN = 950;
			...
			for (const p of paragraphs) {
				if (currentChunk.length + p.length > MAX_LEN) {
					if (currentChunk.trim()) {
						this._addTextChunk(currentChunk.trimEnd(), { hyperlink, citation, latex });
					}
					currentChunk = p + '\n';
				} else {
					currentChunk += p + '\n';
		...
		return this._addTextChunk(text, { hyperlink, citation, latex });
	}

	_addTextChunk(text, { hyperlink = true, citation = true, latex = true } = {}) {
	...
	addVideo(videoUrl, { autoFill = true } = {}) {
		...
		this._submessages.push({
			messageType: 2,
			messageText: '[ CANNOT_LOAD_VIDEO - WA_ ]',
		});
	...
	addProduct(data = {}) {
		...
		this._submessages.push({
			messageType: 2,
			messageText: '[ CANNOT_LOAD_PRODUCT - WA_ ]',
		});
		...
		if (!Array.isArray(data)) {
			this._sections.push(AIRich.newLayout('Single', { text: '\n\n', __typename: 'GenAIMarkdownTextUXPrimitive' }));
		}
	...
	addPost(data = {}) {
		...
		this._submessages.push({
			messageType: 2,
			messageText: '[ CANNOT_LOAD_POST - WA_ ]',
		});
...
function inject(sock) {
	sock.MsgButton = () => new Button(sock);
	sock.MsgButtonV2 = () => new ButtonV2(sock);
	sock.MsgCarousel = () => new Carousel(sock);
	sock.MsgAIRich = () => new AIRich(sock);
	sock.MsgAIRich.AIRich = AIRich;
}

export { Button, ButtonV2, Carousel, AIRich, Toolkit, inject };
